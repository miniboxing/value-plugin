...
trait ValiumInjectTreeTransformer {
  ...
  class TreeInjector(unit: CompilationUnit) extends TreeRewriter(unit) {
    override def rewrite(tree: Tree)(implicit state: State) = {
      case tree @ ValDef(mods, name, tpt @ V(_), rhs) if afterInject(tree.symbol.info.isUnboxedValiumRef) =>
        commit(treeCopy.ValDef(tree, mods, name, tpt.toUnboxedValiumRef, rhs))
      case tree @ DefDef(mods, name, tparams, vparamss, tpt @ V(_), rhs) if !tree.symbol.isConstructor && afterInject(tree.symbol.info.finalResultType.isUnboxedValiumRef) =>
