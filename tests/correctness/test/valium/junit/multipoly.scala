package valium.junit.multipoly
...
class Point[T](val x: T, val y: T) {
  ...
  override def equals(other: Any) = other.isInstanceOf[Point[T]] && other.asInstanceOf[Point[T]].x == x && other.asInstanceOf[Point[T]].y == y
}
...
class MultiPoly {
  @Test def everythingWorks(): Unit = {
    ...
    def identitypoly[T](p1: Point[T]): Point[T] = {
      ...
      var p5 = new Point(3.0, 3.1).asInstanceOf[Point[T]]
      p5 = p2
      p2
    }
    ...
    assert(identitymono(new Point(1.0, 1.1)) == new Point(1.0, 1.1))
    assert(identitypoly(new Point(1.0, 1.1)) == new Point(1.0, 1.1))
    assert(identity2mono(new Point(2.0, 2.1)) == new Point(2.0, 2.1))
    assert(identity2poly(new Point(2.0, 2.1)) == new Point(2.0, 2.1))
    assert(identity3mono(new Point(3.0, 3.1)) == new Point(3.0, 3.1))
    assert(identity3poly(new Point(3.0, 3.1)) == new Point(3.0, 3.1))
  }
}
